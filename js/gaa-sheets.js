(function(){
  ...
  // === GAA Pitch plugin V4: green grass + crisp white markings and posts ===
  const GAA_PITCH_PLUGIN = {
    id: 'gaaPitchV4',
    // Paint grass beneath everything
    beforeDraw(chart){
      ...
      for(let i=0;i<stripes;i++){
        ctx.fillStyle = i%2 ? '#bfe3bf' : '#d2efd2';
        const h = height/stripes;
        ctx.fillRect(left, top + i*h, width, h);
      }
      // Soft border shadow to separate from card background
      ctx.strokeStyle = 'rgba(0,0,0,0.08)';
      ctx.lineWidth = 8;
      ctx.strokeRect(left, top, width, height);
      ctx.restore();
    },
    // Draw white markings & goals on top of points
    afterDatasetsDraw(chart){
      ...
      const code = (codeAttr || 'football').toLowerCase();
      ...
      // "D" arcs at 20m for football
      if(code !== 'hurling'){
        const r = height*0.085;
        const cx = (left+right)/2;
        ctx.beginPath(); ctx.lineWidth = 2; ctx.strokeStyle = '#ffffff';
      ...
      function rect(w,d,up){ const x=(left+right)/2 - w/2; const y = up? top : bottom - d; ctx.strokeRect(x,y,w,d); }
      ctx.lineWidth = 2;
      rect(largeWidth, largeDepth, true);
      rect(smallWidth, smallDepth, true);
      rect(largeWidth, largeDepth, false);
      rect(smallWidth, smallDepth, false);
      ...
      function goal(up){
        ...
        // uprights
        ctx.beginPath();
        ctx.moveTo(cx-gap/2, up ? cbY : bottom); ctx.lineTo(cx-gap/2, up ? top : cbY);
        ctx.moveTo(cx+gap/2, up ? cbY : bottom); ctx.lineTo(cx+gap/2, up ? top : cbY);
        ctx.stroke();
        // crossbar
        ctx.lineWidth = 4;
        ctx.beginPath(); ctx.moveTo(cx-gap/2, cbY); ctx.lineTo(cx+gap/2, cbY); ctx.stroke();
      }
